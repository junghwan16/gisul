...
`skill-test` v1 nails **#1** — cheap, fast, the highest-frequency need. v2
ships **#2** as `skill-test bench`: the same cases, run with and without the
...
- It **adopts the community `evals/cases.yaml` schema** (from the `skill-eval`
  skill) rather than inventing one. Cases stay portable; skill-test is just the
...
```
skill-test                    # discover & run every eval (**/*.eval.yaml, evals/cases.yaml)
skill-test sql                # filter to a skill or a file
skill-test -t "negative"      # filter by case id/substring
skill-test --ci               # non-zero exit on any regression / unwritten case
skill-test --json out.json    # machine-readable results alongside the grid
skill-test bench sql          # A/B with vs without the skill; report the lift
skill-test new sql            # scaffold what's missing: sql/SKILL.md and/or sql.eval.yaml
skill-test lint [target...]   # validate SKILL.md files (errors) + guidance heuristics (warnings)
skill-test fmt [--check]      # conservative SKILL.md frontmatter/whitespace normalizer
```
...
There is a single scaffolding command. `skill-test new <skill>` creates
...
official `skill-creator` runs, packaged as one command: `skill-test bench`.
...
```
skill-test bench [target]           # with vs without, report lift
...
6. **Offline `validate`.** `skill-test validate [target]` parses suites with no
...
8. **Every subcommand silently dropped its options.** The root command was a
   ...
   subcommand, so `skill-test [target]` and bare `skill-test` still work while
...
- **Overlapping installed siblings out-compete the example.** Pass 2 ran on a
  ...
  That is not a CLI defect — it's exactly the over-triggering collision skill-test
...
The source is TypeScript (`tsc` → `dist/`, published as the `skill-test` bin),
