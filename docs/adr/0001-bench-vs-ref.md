...
`skill-test bench` answers "does this skill help vs. no skill at all" — the
...
```
$ skill-test bench code-review --vs main
...
```
skill-test bench [target] --vs <ref>
```
...
- **Not a general git-diff tool.** No prose-diff rendering; `skill-test` reports
...
gap — a future `skill-test capture <skill> "<prompt>"` — left as its own piece
